...
type Search struct {
	Id          int64  `json:"id"`
	ProviceId   int64  `json:"provice_id"`
...
type ProvinceList struct {
	...
	Provinces []Province `json:"Province"`
}
...
func New(db *sql.DB, level int) *ProvinceList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &ProvinceList{DB: db, Total: 0, Provinces: make([]Province, 0), Level: level}
}
...
func NewUrl(url string, level int) *ProvinceList {
	var err error
...
func (r *ProvinceList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
	...
	if s.ProviceId != 0 {
		where += " and provice_id=" + fmt.Sprintf("%d", s.ProviceId)
	}

	if s.ProviceName != "" {
		where += " and provice_name='" + s.ProviceName + "'"
	}
	...
	qrySql := fmt.Sprintf("Select count(1) as total from j_position_provice   where 1=1 %s", where)
	if r.Level == DEBUG {
...
func (r ProvinceList) Get(s Search) (*Province, error) {
	var where string
	l := time.Now()
	...
	qrySql := fmt.Sprintf("Select id,provice_id,provice_name,version from j_position_provice where 1=1 %s ", where)
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	rows, err := r.DB.Query(qrySql)
	...
	if !rows.Next() {
		...
		err := rows.Scan(&p.Id, &p.ProviceId, &p.ProviceName, &p.Version)
		if err != nil {
			log.Println(SQL_ERROR, err.Error())
			return nil, err
		}
...
func (r *ProvinceList) GetList(s Search) ([]Province, error) {
	var where string
	l := time.Now()
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		...
		qrySql = fmt.Sprintf("Select id,provice_id,provice_name,version from j_position_provice where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	var p Province
	for rows.Next() {
		rows.Scan(&p.Id, &p.ProviceId, &p.ProviceName, &p.Version)
...
func (r *ProvinceList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r ProvinceList) Insert(p Province) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  j_position_provice(provice_id,provice_name,version)  values(?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.ProviceId, p.ProviceName, p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r ProvinceList) InsertEntity(p Province, tr *sql.Tx) error {
	l := time.Now()
	...
	if p.ProviceId != 0 {
		colNames += "provice_id,"
		colTags += "?,"
		valSlice = append(valSlice, p.ProviceId)
	}

	if p.ProviceName != "" {
		colNames += "provice_name,"
		colTags += "?,"
		valSlice = append(valSlice, p.ProviceName)
	}
	...
	exeSql := fmt.Sprintf("Insert into  j_position_provice(%s)  values(%s)", colNames, colTags)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r ProvinceList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r ProvinceList) UpdataEntity(keyNo string, p Province, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
	...
	exeSql := fmt.Sprintf("update  j_position_provice  set %s  where id=? ", colNames)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r ProvinceList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	...
	valSlice = append(valSlice, keyNo)
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update j_position_provice set %s where id=?", colNames)
	if r.Level == DEBUG {
...
func (r ProvinceList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  j_position_provice  where id=?")
	if r.Level == DEBUG {
