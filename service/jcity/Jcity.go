...
func New(db *sql.DB, level int) *CityList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &CityList{DB: db, Total: 0, Citys: make([]City, 0), Level: level}
}
...
func NewUrl(url string, level int) *CityList {
	var err error
...
func (r *CityList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
	...
	if s.ProvinceId != 0 {
		where += " and province_id=" + fmt.Sprintf("%d", s.ProvinceId)
	}
	...
	if s.CityName != "" {
		where += " and city_name='" + s.CityName + "'"
	}
	...
	qrySql := fmt.Sprintf("Select count(1) as total from j_position_city   where 1=1 %s", where)
	if r.Level == DEBUG {
...
func (r CityList) Get(s Search) (*City, error) {
	var where string
	l := time.Now()
	...
	qrySql := fmt.Sprintf("Select id,province_id,city_id,city_name,version from j_position_city where 1=1 %s ", where)
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	rows, err := r.DB.Query(qrySql)
	...
	if !rows.Next() {
		...
		err := rows.Scan(&p.Id, &p.ProvinceId, &p.CityId, &p.CityName, &p.Version)
		if err != nil {
			log.Println(SQL_ERROR, err.Error())
			return nil, err
		}
...
func (r *CityList) GetList(s Search) ([]City, error) {
	var where string
	l := time.Now()
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		...
		qrySql = fmt.Sprintf("Select id,province_id,city_id,city_name,version from j_position_city where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	var p City
	for rows.Next() {
		rows.Scan(&p.Id, &p.ProvinceId, &p.CityId, &p.CityName, &p.Version)
		r.Citys = append(r.Citys, p)
	}
	log.Println(SQL_ELAPSED, r)
	if r.Level == DEBUG {
		log.Println(SQL_ELAPSED, time.Since(l))
...
func (r *CityList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r CityList) Insert(p City) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  j_position_city(province_id,city_id,city_name,version)  values(?,?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.ProvinceId, p.CityId, p.CityName, p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r CityList) InsertEntity(p City, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)

	if p.ProvinceId != 0 {
		colNames += "province_id,"
		colTags += "?,"
		valSlice = append(valSlice, p.ProvinceId)
	}
	...
	if p.CityName != "" {
		colNames += "city_name,"
		colTags += "?,"
		valSlice = append(valSlice, p.CityName)
	}
	...
	exeSql := fmt.Sprintf("Insert into  j_position_city(%s)  values(%s)", colNames, colTags)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r CityList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r CityList) UpdataEntity(keyNo string, p City, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
	...
	exeSql := fmt.Sprintf("update  j_position_city  set %s  where id=? ", colNames)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r CityList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	...
	valSlice = append(valSlice, keyNo)
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update j_position_city set %s where id=?", colNames)
	if r.Level == DEBUG {
...
func (r CityList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  j_position_city  where id=?")
	if r.Level == DEBUG {
