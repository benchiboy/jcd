...
type Search struct {
	Id         int64  `json:"id"`
	TipType    int64  `json:"tip_type"`
	UserName   string `json:"user_name"`
	IdNo       string `json:"id_no"`
	Phone      string `json:"phone"`
	TipNo      string `json:"tip_no"`
...
type TiplistList struct {
	DB       *sql.DB
	Level    int
	Total    int       `json:"total"`
	Tiplists []Tiplist `json:"Tiplist"`
}
...
func New(db *sql.DB, level int) *TiplistList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &TiplistList{DB: db, Total: 0, Tiplists: make([]Tiplist, 0), Level: level}
}
...
func NewUrl(url string, level int) *TiplistList {
	var err error
...
func (r *TiplistList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
	...
	if s.TipType != 0 {
		where += " and tip_type=" + fmt.Sprintf("%d", s.TipType)
	}
	...
	if s.TipNo != "" {
		where += " and tip_no='" + s.TipNo + "'"
	}

	if s.TipMsg != "" {
		where += " and tip_msg='" + s.TipMsg + "'"
	}
	...
	qrySql := fmt.Sprintf("Select count(1) as total from b_tiplist   where 1=1 %s", where)
	if r.Level == DEBUG {
...
func (r TiplistList) Get(s Search) (*Tiplist, error) {
	var where string
	l := time.Now()
	...
	qrySql := fmt.Sprintf("Select id,tip_type,user_name,id_no,phone,tip_no,tip_msg,insert_time,update_time,update_user,version from b_tiplist where 1=1 %s ", where)
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	rows, err := r.DB.Query(qrySql)
	...
	if !rows.Next() {
		...
		err := rows.Scan(&p.Id, &p.TipType, &p.UserName, &p.IdNo, &p.Phone, &p.TipNo, &p.TipMsg, &p.InsertTime, &p.UpdateTime, &p.UpdateUser, &p.Version)
		if err != nil {
			log.Println(SQL_ERROR, err.Error())
			return nil, err
		}
...
func (r *TiplistList) GetList(s Search) ([]Tiplist, error) {
	var where string
	l := time.Now()
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		...
		qrySql = fmt.Sprintf("Select id,tip_type,user_name,id_no,phone,tip_no,tip_msg,insert_time,update_time,update_user,version from b_tiplist where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	var p Tiplist
	for rows.Next() {
		rows.Scan(&p.Id, &p.TipType, &p.UserName, &p.IdNo, &p.Phone, &p.TipNo, &p.TipMsg, &p.InsertTime, &p.UpdateTime, &p.UpdateUser, &p.Version)
		r.Tiplists = append(r.Tiplists, p)
	}
	log.Println(SQL_ELAPSED, r)
	if r.Level == DEBUG {
		log.Println(SQL_ELAPSED, time.Since(l))
	}
	return r.Tiplists, nil
}
...
func (r *TiplistList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r TiplistList) Insert(p Tiplist) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  b_tiplist(tip_type,user_name,id_no,phone,tip_no,tip_msg,update_user,version)  values(?,?,?,?,?,?,?,?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.TipType, p.UserName, p.IdNo, p.Phone, p.TipNo, p.TipMsg, p.UpdateUser, p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r TiplistList) InsertEntity(p Tiplist, tr *sql.Tx) error {
	l := time.Now()
	...
	if p.TipType != 0 {
		colNames += "tip_type,"
		colTags += "?,"
		valSlice = append(valSlice, p.TipType)
	}
	...
	if p.TipNo != "" {
		colNames += "tip_no,"
		colTags += "?,"
		valSlice = append(valSlice, p.TipNo)
	}

	if p.TipMsg != "" {
		colNames += "tip_msg,"
		colTags += "?,"
		valSlice = append(valSlice, p.TipMsg)
	}
	...
	exeSql := fmt.Sprintf("Insert into  b_tiplist(%s)  values(%s)", colNames, colTags)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r TiplistList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r TiplistList) UpdataEntity(keyNo string, p Tiplist, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
	...
	exeSql := fmt.Sprintf("update  b_tiplist  set %s  where id=? ", colNames)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r TiplistList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	...
	valSlice = append(valSlice, keyNo)
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update b_tiplist set %s where id=?", colNames)
	if r.Level == DEBUG {
...
func (r TiplistList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  b_tiplist  where id=?")
	if r.Level == DEBUG {
