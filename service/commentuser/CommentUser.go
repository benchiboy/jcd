...
type CommentUserList struct {
	DB           *sql.DB
	Level        int
	Total        int           `json:"total"`
	CommentUsers []CommentUser `json:"CommentUser"`
}
...
type Form struct {
	Form CommentUser `json:"CommentUser"`
}
...
func New(db *sql.DB, level int) *CommentUserList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &CommentUserList{DB: db, Total: 0, CommentUsers: make([]CommentUser, 0), Level: level}
}
...
func NewUrl(url string, level int) *CommentUserList {
	var err error
...
func (r *CommentUserList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
	...
	if s.ActionType != 0 {
		where += " and action_type=" + fmt.Sprintf("%d", s.ActionType)
	}
	...
	qrySql := fmt.Sprintf("Select count(1) as total from b_comment_user   where 1=1 %s", where)
	if r.Level == DEBUG {
...
func (r CommentUserList) Get(s Search) (*CommentUser, error) {
	var where string
	l := time.Now()
...
func (r *CommentUserList) GetList(s Search) ([]CommentUser, error) {
	var where string
	l := time.Now()
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		...
		qrySql = fmt.Sprintf("Select id,action_type,comm_no,user_id,insert_time,version from b_comment_user where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	var p CommentUser
	for rows.Next() {
		rows.Scan(&p.Id, &p.ActionType, &p.CommNo, &p.UserId, &p.InsertTime, &p.Version)
		r.CommentUsers = append(r.CommentUsers, p)
	}
	log.Println(SQL_ELAPSED, r)
	if r.Level == DEBUG {
		log.Println(SQL_ELAPSED, time.Since(l))
	}
	return r.CommentUsers, nil
}
...
func (r *CommentUserList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r CommentUserList) Insert(p CommentUser) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  b_comment_user(action_type,comm_no,user_id,version)  values(?,?,?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.ActionType, p.CommNo, p.UserId, p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r CommentUserList) InsertEntity(p CommentUser, tr *sql.Tx) error {
	l := time.Now()
	...
	exeSql := fmt.Sprintf("Insert into  b_comment_user(%s)  values(%s)", colNames, colTags)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r CommentUserList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r CommentUserList) UpdataEntity(keyNo string, p CommentUser, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
	...
	exeSql := fmt.Sprintf("update  b_comment_user  set %s  where id=? ", colNames)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r CommentUserList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	...
	valSlice = append(valSlice, keyNo)
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update b_comment_user set %s where id=?", colNames)
	if r.Level == DEBUG {
...
func (r CommentUserList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  b_comment_user  where id=?")
	if r.Level == DEBUG {
