...
func New(db *sql.DB, level int) *VillageList {
	if db == nil {
		log.Println(SQL_SELECT, "Database is nil")
		return nil
	}
	return &VillageList{DB: db, Total: 0, Villages: make([]Village, 0), Level: level}
}
...
func NewUrl(url string, level int) *VillageList {
	var err error
...
func (r *VillageList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
	...
	if s.VillageId != 0 {
		where += " and village_id=" + fmt.Sprintf("%d", s.VillageId)
	}

	if s.VillageName != "" {
		where += " and village_name='" + s.VillageName + "'"
	}
	...
	qrySql := fmt.Sprintf("Select count(1) as total from j_position_village   where 1=1 %s", where)
	if r.Level == DEBUG {
...
func (r VillageList) Get(s Search) (*Village, error) {
	var where string
	l := time.Now()
	...
	qrySql := fmt.Sprintf("Select id,town_id,village_id,village_name,version from j_position_village where 1=1 %s ", where)
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	rows, err := r.DB.Query(qrySql)
	...
	if !rows.Next() {
		...
		err := rows.Scan(&p.Id, &p.TownId, &p.VillageId, &p.VillageName, &p.Version)
		if err != nil {
			log.Println(SQL_ERROR, err.Error())
			return nil, err
		}
...
func (r *VillageList) GetList(s Search) ([]Village, error) {
	var where string
	l := time.Now()
	...
	if s.PageSize == 0 && s.PageNo == 0 {
		...
		qrySql = fmt.Sprintf("Select id,town_id,village_id,village_name,version from j_position_village where 1=1 %s Limit %d offset %d", where, s.PageSize, (s.PageNo-1)*s.PageSize)
	}
	if r.Level == DEBUG {
		log.Println(SQL_SELECT, qrySql)
	}
	...
	var p Village
	for rows.Next() {
		rows.Scan(&p.Id, &p.TownId, &p.VillageId, &p.VillageName, &p.Version)
		r.Villages = append(r.Villages, p)
	}
	log.Println(SQL_ELAPSED, r)
	if r.Level == DEBUG {
		log.Println(SQL_ELAPSED, time.Since(l))
...
func (r *VillageList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r VillageList) Insert(p Village) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  j_position_village(town_id,village_id,village_name,version)  values(?,?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.TownId, p.VillageId, p.VillageName, p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r VillageList) InsertEntity(p Village, tr *sql.Tx) error {
	l := time.Now()
	...
	if p.VillageId != 0 {
		colNames += "village_id,"
		colTags += "?,"
		valSlice = append(valSlice, p.VillageId)
	}

	if p.VillageName != "" {
		colNames += "village_name,"
		colTags += "?,"
		valSlice = append(valSlice, p.VillageName)
	}
	...
	exeSql := fmt.Sprintf("Insert into  j_position_village(%s)  values(%s)", colNames, colTags)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r VillageList) InsertMap(m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r VillageList) UpdataEntity(keyNo string, p Village, tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
	...
	exeSql := fmt.Sprintf("update  j_position_village  set %s  where id=? ", colNames)
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
...
func (r VillageList) UpdateMap(keyNo string, m map[string]interface{}, tr *sql.Tx) error {
	l := time.Now()
	...
	valSlice = append(valSlice, keyNo)
	colNames = strings.TrimRight(colNames, ",")
	updateSql := fmt.Sprintf("Update j_position_village set %s where id=?", colNames)
	if r.Level == DEBUG {
...
func (r VillageList) Delete(keyNo string, tr *sql.Tx) error {
	l := time.Now()
	delSql := fmt.Sprintf("Delete from  j_position_village  where id=?")
	if r.Level == DEBUG {
